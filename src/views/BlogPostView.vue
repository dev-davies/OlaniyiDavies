        ...
        <div class="col-lg-8 col-md-10">
          ...
          <div v-if="post.frontmatter.tags?.length" class="mt-5 pt-4 border-top">
            ...
            <div class="d-flex flex-wrap gap-2">
              <span v-for="tag in post.frontmatter.tags" :key="tag" class="badge bg-body-secondary text-body-emphasis border px-3 py-2">
                #{{ tag }}
              </span>
            </div>
          </div>
          ...
          <div class="mt-4 d-flex align-items-center gap-3">
            ...
            <a :href="`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.frontmatter.title)}&url=${encodeURIComponent(currentUrl)}`" target="_blank" class="btn btn-sm share-btn rounded-pill px-3 d-inline-flex align-items-center">
              <Twitter :size="16" class="me-1" /> Twitter
            </a>
            <a :href="`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(currentUrl)}`" target="_blank" class="btn btn-sm share-btn rounded-pill px-3 d-inline-flex align-items-center">
              <Linkedin :size="16" class="me-1" /> LinkedIn
            </a>
          </div>

          <!-- What do you think Section -->
          <div class="mt-5 bg-body-tertiary border p-4 rounded-4 text-center">
            <h4 class="fw-bold mb-2 text-body-emphasis">What do you think?</h4>
            <p class="text-body-secondary mb-3">Do you disagree, have additions, or just want to discuss? I'd love to hear from you.</p>
          ...
          <div class="mt-5 pt-4 border-top d-flex justify-content-between">
             <RouterLink v-if="adjacentPosts.prev" :to="`/blog/${adjacentPosts.prev.slug}`" class="text-decoration-none w-50 pe-3 transition-hover">
               <div class="text-body-secondary small mb-1">← Previous Post</div>
               <div class="fw-bold text-body-emphasis">{{ adjacentPosts.prev.frontmatter.title }}</div>
             </RouterLink>
             ...
             <RouterLink v-if="adjacentPosts.next" :to="`/blog/${adjacentPosts.next.slug}`" class="text-decoration-none text-end w-50 ps-3 transition-hover">
               <div class="text-body-secondary small mb-1">Next Post →</div>
               <div class="fw-bold text-body-emphasis">{{ adjacentPosts.next.frontmatter.title }}</div>
             </RouterLink>
